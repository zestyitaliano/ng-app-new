...
export const RatioDisplay: React.FC<RatioDisplayProps> = ({ width, height, isValid }) => {
  const decimalRatio = isValid ? (width / height).toFixed(4) : '—';
  const simpleRatio = isValid ? getSimplifiedRatio(width, height) : '—';

  return (
    <div className="flex flex-col items-center justify-center py-4 px-6 bg-secondary rounded-none w-full sm:w-auto min-w-[140px]">
